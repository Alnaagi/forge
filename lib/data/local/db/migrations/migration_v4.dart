...
class MigrationV4 {
  ...
  static const indexStatements = <String>[
    ...
    'CREATE INDEX IF NOT EXISTS health_status_logs_logged_at_idx ON health_status_logs_table (logged_at DESC)',
    'CREATE INDEX IF NOT EXISTS health_status_logs_type_logged_at_idx ON health_status_logs_table (type, logged_at DESC)',
    'CREATE INDEX IF NOT EXISTS health_profiles_updated_at_idx ON health_profiles_table (updated_at DESC)',
  ];
  ...
  static Future<void> onUpgrade(
    ...
    if (from < 3) {
      await migrator.createTable(database.healthProfilesTable);
      await migrator.createTable(database.healthStatusLogsTable);
    }
    if (from < 4) {
      await migrator.addColumn(
        database.remindersTable,
        database.remindersTable.isUserDisabled,
      );
    }
